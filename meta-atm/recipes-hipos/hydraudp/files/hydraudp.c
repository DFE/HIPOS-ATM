...
#define MAX_STREAM 8                      /**< maximum supported stream ID (vin/1..8, dv/1..8), the real maximum depends on the remote system */
...
typedef struct /* inherits struct drtp_stream_handle_t */ 
{
    ...
    gint net_errors;                          /**< detected net errors (statistics only) */
...
static int fb_handle;                         /**< frame buffer file descriptor (server only) */
...
static gboolean trace_fames[MAX_STREAM];      /**< stream trace flag (server only) */
...
static void log_handler_cb (const gchar* log_domain, GLogLevelFlags log_level, const gchar *message, gpointer user_data)
{
	...
	if ((s_server || s_journal) && !s_stdout)
	{
 ...
 * is an abort (g_error). Main reasons are frozen hardware (mainly decoder on bit errors) 
 * and memory problems.
 *
...
static void on_error(GstBus *bus, GstMessage *message, gpointer _stream)
{
	...
	if (GST_MESSAGE_ERROR == GST_MESSAGE_TYPE(message))
	{
		...
		g_error("gst error message: vin/%u element %s: %s (%s)\n", stream->vin, GST_OBJECT_NAME(message->src), err->message, (dbg_info ? dbg_info : ""));
		g_error_free (err);
		g_free (dbg_info);
	} else {
		g_critical("%s vin/%u called with type %d\n", __func__, stream->vin, GST_MESSAGE_TYPE(message));
	}
}
...
static void remote_vin_stop(unsigned quadrant)
{
	if (quadrant >= MAX_QUADRANT) { g_error("%s illegal quadrant %u", __func__, quadrant); return; } /* impl. sanity */
	...
	if (stream && stream->running)
	{
		...
		if (DRTP_SUCCESS != ret) {
			...
			g_message("%s quadrant %u stopped rtp stream vin/%u after %u frames %u sec %.1f fps %u udp  errors", __func__, quadrant, stream->vin, stream->frame_count, diff, fps, stream->net_errors);
		}
...
static void remote_vin_start(unsigned quadrant, const char* ipaddr, unsigned vin, unsigned x, unsigned y, unsigned width, unsigned height) 
{
	if (vin-1 >= MAX_STREAM) { g_error("%s illegal vin/%u", __func__, vin); } /* impl. sanity */
	...
	{ /* sanity */
		...
		remote_vin_stop(quadrant); 
	} 
	g_debug("%s: quadrant %u %s:vin/%u %ux%u %ux%u", __func__, quadrant, ipaddr, vin, x, y, width, height);
	...
	{  /* on gstreamer based systems is better to get the full pal resolution because scaling is here the bottle nack */
		g_debug("%s starting quadrant %u %s vin/%u on display [%u,%u] width %u height %u (704x576)", __func__, quadrant, stream->ipaddr, vin, x, y, width, height);
		CHECK_TRUE(DRTP_SUCCESS == drtp_udp_stream_start(&stream->sh, rtp_frame_cb, rtp_memory_cb, stream->ipaddr, vin, 704, 576));
	} else { /* reduce the resolution with old hipox devices, here the encoder is the bottle neck */
		if (width == WIDTH)
		{
			g_debug("%s starting quadrant %u %s vin/%u on display [%u,%u] width %u height %u (704x288)", __func__, quadrant, stream->ipaddr, vin, x, y, width, height);
			CHECK_TRUE(DRTP_SUCCESS == drtp_udp_stream_start(&stream->sh, rtp_frame_cb, rtp_memory_cb, stream->ipaddr, vin, 704, 288));
		} else { // quad
			g_debug("%s starting quadrant %u %s vin/%u on display [%u,%u] width %u height %u (352x288)", __func__, quadrant, stream->ipaddr, vin, x, y, width, height);
			CHECK_TRUE(DRTP_SUCCESS == drtp_udp_stream_start(&stream->sh, rtp_frame_cb, rtp_memory_cb, stream->ipaddr, vin, 352, 288));
		}
	}
}
...
static void view_1(const char* ipaddr, unsigned vin)
{
	...
	if (vin-1 >= MAX_STREAM) { g_critical("%s illegal vin/%u requested - black screen"); blank_area(0, 0, WIDTH, HEIGHT, 0); }
	else { remote_vin_start(0, ipaddr, vin, 0, 0, WIDTH, HEIGHT); }
}
...
static void view_4(const char* ipaddr, unsigned vin1, unsigned vin2, unsigned vin3, unsigned vin4)
{
	...
	unsigned h = HEIGHT / 2;
	view_0(); /* in case it's running */
	if (vin1-1 >= MAX_STREAM) { g_critical("%s illegal vin/%u requested - black quadrant 0"); blank_area(0, 0, w, h, 0); }
	else { remote_vin_start(0, ipaddr, vin1, 0, 0, w, h); }
	if (vin2-1 >= MAX_STREAM) { g_critical("%s illegal vin/%u requested - black quadrant 1"); blank_area(w, 0, w, h, 0); }
	else { remote_vin_start(1, ipaddr, vin2, w, 0, w, h); }
	if (vin3-1 >= MAX_STREAM) { g_critical("%s illegal vin/%u requested - black quadrant 2"); blank_area(0, h, w, h, 0); }
	else { remote_vin_start(2, ipaddr, vin3, 0, h, w, h); }
	if (vin4-1 >= MAX_STREAM) { g_critical("%s illegal vin/%u requested - black quadrant 2"); blank_area(w, h, w, h, 0); }
	else { remote_vin_start(3, ipaddr, vin4, w, h, w, h); }
}
...
static gboolean server_cmd(gchar* cmd)
{
	switch(cmd[0])
	{
		...
		case 'D':
			{
				unsigned cam;
				if ((1 != sscanf(cmd+1, "%u", &cam)) || (cam-1 >= MAX_STREAM)) { g_warning("illegal server command args '%s' for stream debugging", cmd+1); break; }
				trace_fames[cam-1] = !trace_fames[cam-1];
				g_message("stream debugging vin/%u: %d", cam, trace_fames[cam-1]);
			} break;
		case 'd':
			{
				if (1 != sscanf(cmd+1, "%u", &s_debug)) { g_warning("illegal server command args '%s' for debug level", cmd+1); break; }
				g_message("stream debug level set to %s (%d)", log_level_to_string(G_LOG_LEVEL_ERROR << s_debug), s_debug);
			} break;
		case 'S':
			return FALSE;
		default:
    ...
    if (condition & G_IO_IN)
    {
		...
		switch(status)
		{
			...
			case G_IO_STATUS_ERROR:
				g_debug("%s: ERROR", __func__);
			case G_IO_STATUS_EOF:
				g_debug("%s: EOF", __func__);
			default:
				g_debug("%s: status ??? (0x%X)", __func__, status);
				break;
		}
...
{
	g_message("Caught signal %d (ign)", signum);
}
...
static int server_main(GOptionContext *context, int argc, char**argv)
{
	...
	if (fb_handle >= 0)
	{
		...
		{
			g_error("%s handle %d ioctl(FBIOGET_VSCREENINFO) failed: %s(%d)", FRAMEBUFFER, fb_handle, strerror(errno), errno);
		}
		...
		g_error("frame buffer %s open failed: %s(%d)", FRAMEBUFFER, strerror(errno), errno);
	}
...
#endif
	g_info("display resolution %ux%u video area [0, 0, %u, %u]", fb_vinfo.xres, fb_vinfo.yres, WIDTH, HEIGHT);
...
{
	g_message("Caught signal %d switch off", signum);
	const char* buf = "0\n";
	int len = strlen(buf), l;
	if (len != (l = write(s_fifo, buf, len)))
	{
		g_critical("%s: write(%2s (%u) to %s failed: %s(%d/%d)", __func__, RUN_NAME ".fifo", buf, len, strerror(errno), errno, l);
	}
...
static int write_fifo(gchar* buf)
{
	...
	if (len != (l = write(s_fifo, buf, len)))
	{
		...
		g_debug("%s: write(%.*s (%u)) to %s", __func__, len-1, buf, len, RUN_NAME ".fifo");
		ret = len;
	}
	g_free(buf);
	return ret;
...
static int adapter_main(GOptionContext *context, int argc, char**argv)
{
	...
	switch (argc)
	{
		case 2: write_fifo(g_strdup_printf("0\n", argv[1])); do_wait = FALSE; break;
		case 3: write_fifo(g_strdup_printf("1%s#%s\n", argv[1], argv[2])); break;
		case 6: write_fifo(g_strdup_printf("4%s#%s#%s#%s#%s\n", argv[1], argv[2], argv[3], argv[4], argv[5])); break;
		case 0:
				g_critical("%s: missing program name, IP and quadrant camera(s) - black screen", __func__);
				write_fifo(g_strdup("B\n"));
				break;
		case 1:
				g_message("%s: shutdown server", argv[0]);
				write_fifo(g_strdup("S\n"));
				do_wait = FALSE;
				break;
		default:
	...
	while (do_wait)
	{
		g_message("%s: alive - wait for signal to stop video output", argv[0]);
		sleep(60);
	}
	return 0;
}
	...
	if (g_file_get_contents( RUN_NAME ".pid", &spid, 0, 0) && (1 == sscanf(spid, "%u", &server_pid)))
	{
		if (g_file_test((proc = g_strdup_printf("/proc/%u", server_pid)), G_FILE_TEST_IS_DIR)) /* check running */
		{
			if (s_server)
			{
				...
				g_message("hydra vout service with PID:%u active", server_pid);
			}
		} else {
			g_message("hydra vout old service PID:%u not active", server_pid);
			server_pid = 0;
		}
	} else { 
		g_message("hydra vout no service active");
		server_pid = 0;
	}
...
static GOptionEntry s_main_opts[] =
{
	...
	{ "journal",        'j', 0, G_OPTION_ARG_NONE, &s_journal,      "trace the adapter to journal instead of stderr", NULL},
	{ "stderr",         's', 0, G_OPTION_ARG_NONE, &s_stdout,       "all traces to stderr (ignores --journal)", NULL},
